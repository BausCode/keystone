...
exports = module.exports = {
	...
	upload: function(req, res) {
		...
		if(req.files && req.files.file){
			...
			var editorOptions = keystone.get('wysiwyg additional options');

			var file = req.files.file,
				path = (s3Config.s3path ? s3Config.s3path + '/' : '') + 
					(editorOptions.images_upload_base_path ? editorOptions.images_upload_base_path + '/' : '');
				...
				var sendResult = function () {
					...
					if (s3Response) {
						if (s3Response.statusCode !== 200) {
							...
							return res.send({ image: { url: 'https://s3.amazonaws.com/' + s3Config.bucket + '/' + path + file.name } });
						}
					}
				};
